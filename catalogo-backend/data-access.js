...
// Validate UUIDs generated by PostgreSQL-backed Prisma models.
...
// Translate the service filter DSL to Prisma where conditions.
function mapQuery(modelName, queryFilter) {
    if (!queryFilter || typeof queryFilter !== 'object') return {};
...
// Translate service sort options to Prisma orderBy.
...
// Map included Prisma relations to the service's public model shape.
...
// Chainable query wrapper used by the service layer.
class QueryCompat {
...
// Create the mutable model shape used by the existing service layer.
...
function makeModel(modelName, prismaModelName) {
    ...
    ModelConstructor.find = function(queryFilter = {}) {
        return new QueryCompat(modelName, async (queryObj) => {
            const where = mapQuery(modelName, queryFilter);
    ...
    ModelConstructor.findOne = function(queryFilter = {}) {
        return new QueryCompat(modelName, async (queryObj) => {
            const where = mapQuery(modelName, queryFilter);
            const prismaArgs = { where };
    ...
    ModelConstructor.updateOne = async function(queryFilter, updateObj) {
        const where = mapQuery(modelName, queryFilter);
    ...
    ModelConstructor.updateMany = async function(queryFilter, updateObj) {
        const where = mapQuery(modelName, queryFilter);
        const updateData = updateObj.$set || updateObj;
    ...
    ModelConstructor.deleteOne = async function(queryFilter) {
        const where = mapQuery(modelName, queryFilter);
    ...
    ModelConstructor.deleteMany = async function(queryFilter) {
        const where = mapQuery(modelName, queryFilter);
        if (modelName === 'Product' || modelName === 'Category') {
            const result = await prisma[prismaModelName].updateMany({
                where,
    ...
    ModelConstructor.findOneAndUpdate = async function(queryFilter, updateObj, options = {}) {
        const where = mapQuery(modelName, queryFilter);
    ...
    ModelConstructor.findOneAndDelete = async function(queryFilter) {
        const where = mapQuery(modelName, queryFilter);
        const record = await prisma[prismaModelName].findFirst({ where });
    ...
    ModelConstructor.countDocuments = async function(queryFilter = {}) {
        const where = mapQuery(modelName, queryFilter);
        return prisma[prismaModelName].count({ where });
    };

    ModelConstructor.exists = async function(queryFilter = {}) {
        const where = mapQuery(modelName, queryFilter);
